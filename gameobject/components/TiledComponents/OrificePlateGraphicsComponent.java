 ...
 * @author marci
 */
public class OrificePlateGraphicsComponent extends TiledGraphicsComponent {

    private static final String TAG = OrificePlateGraphicsComponent.class.getSimpleName();
    
    private TextureMapObject tmo;    
    private TextureRegion textureRegion;
    
    public OrificePlateGraphicsComponent(float x, float y){
        
        Texture texture = new Texture(Gdx.files.internal("img/orifice_plate_128x64.png"));
    ...
    public void render(GameObject obj, SpriteBatch batch) {
        ...
        drawString(obj, batch, "F(m3/h): "+str, 8, 24);
        
        number = obj.getPhysics().getEntrada(0).getPressure()/101325f;            
        str = String.format("%.2f", number);
        drawString(obj, batch, "Pin(atm): "+str, 8, 40);

        number = obj.getPhysics().getSaida(0).getPressure()/101325f;
        str = String.format("%.2f", number);
        drawString(obj, batch, "Pout(atm): "+str, 8, 56);
