 ...
 * @author marci
 */
public class ElectricHeaterGraphicsComponent extends TiledGraphicsComponent {
    
    private static final String TAG = ElectricHeaterGraphicsComponent.class.getSimpleName();
    
    private TextureMapObject tmo;    
    private TextureRegion textureRegion;
    
    public ElectricHeaterGraphicsComponent(float x, float y){
        
        Texture texture = new Texture(Gdx.files.internal("img/electric_heater_448x128.png"));
    ...
    public void render(GameObject obj, SpriteBatch batch) {
        ...
        drawString(obj, batch, "Q(m3/h): "+str, 8, 24);
        
        number = obj.getPhysics().getEntrada(0).getPressure()/101325;
        str = String.format("%.2f", number);
        drawString(obj, batch, "P(atm): "+str, 8, 40);

        number = obj.getPhysics().getEntrada(0).getTemperature() - 273.15f;
        str = String.format("%.2f", number);
        drawString(obj, batch, "T_in(oC): "+str, 8, 56);
        
        number = obj.getPhysics().getSaida(0).getTemperature() - 273.15f;
        str = String.format("%.2f", number);
        drawString(obj, batch, "T_out(oC): "+str, 8, 72);
