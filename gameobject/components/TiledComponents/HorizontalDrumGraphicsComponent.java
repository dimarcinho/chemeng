 ...
 * @author marci
 */
public class HorizontalDrumGraphicsComponent extends TiledGraphicsComponent {

    private static final String TAG = HorizontalDrumGraphicsComponent.class.getSimpleName();
    
    private TextureMapObject tmo;    
    private TextureRegion textureRegion;
    
    public HorizontalDrumGraphicsComponent(float x, float y){
        
        Texture texture = new Texture(Gdx.files.internal("img/horizontal_drum_512x128.png"));
    ...
    public void render(GameObject obj, SpriteBatch batch) {
        ...
        str = String.format("%.2f", number);
        drawString(obj, batch, "F_heavy(m3/h): "+str, 8, 40);            
        
        number = obj.getPhysics().getSaida(1).getFlow();
        str = String.format("%.2f", number);
        drawString(obj, batch, "F_light(m3/h): "+str, 8, 56);
